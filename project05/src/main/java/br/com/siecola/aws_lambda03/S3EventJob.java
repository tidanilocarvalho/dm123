...
public class S3EventJob implements RequestHandler<S3Event, String> {
    ...
    private void saveInvoice(Invoice invoice) {
        final AmazonDynamoDBClient ddbClient = new AmazonDynamoDBClient(new EnvironmentVariableCredentialsProvider());

        ddbClient.withRegion(Regions.US_WEST_1);
    ...
    private String downloadObject(String bucketName, String objectKey) throws IOException {
        final AmazonS3Client amazonS3Client = new AmazonS3Client(new EnvironmentVariableCredentialsProvider());

        amazonS3Client.withRegion(Regions.US_WEST_1);
